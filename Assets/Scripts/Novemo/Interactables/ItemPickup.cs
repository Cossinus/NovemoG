...
{
    public class ItemPickup : Interactables.Interactable
    {
        ...
        private void PickUp()
        {
            ...
            if (amount < tmpAmount)
                EventLog.Instance.RaiseEventLog($"You picked up {tmpAmount - amount} {Metrics.GetStringWithRarity(item.itemRarity, item.itemName)}");
